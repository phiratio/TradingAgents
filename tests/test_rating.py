"""Tests for the shared 5-tier rating heuristic.

The Portfolio Manager renders its decision to markdown that always contains
a ``**Rating**: X`` header, so the deterministic heuristic in
``tradingagents.rating`` is sufficient to extract the rating downstream —
...
class TestParseRating:
    ...
    def test_rendered_pm_markdown_shape(self):
        # The exact shape produced by the PM decision markdown must always parse.
        text = (
            "**Rating**: Buy\n\n"
