...
class TradingMemoryLog:
    ...
    def store_decision(
        ...
        """Append pending entry at the end of a run. No LLM call.

        Returns True when a new entry was written, False when logging is
        disabled or an entry for (trade_date, ticker) is already pending.
        """
    ...
    def update_with_outcome(
        ...
        Returns True when an entry was updated, False when logging is disabled
        or no matching pending entry exists.
        """
        if not self._log_path or not self._log_path.exists():
            return False
    ...
    def batch_update_with_outcomes(self, updates: list[dict]) -> int:
        ...
        applied = len(updates) - len(update_map)
        new_blocks = self._apply_rotation(new_blocks)
        new_text = self._SEPARATOR.join(new_blocks)
        tmp_path = self._log_path.with_suffix(".tmp")
        tmp_path.write_text(new_text, encoding="utf-8")
